...
const int servoPins[12] = {15, 18, 19, 17, 20, 21, 14, 9, 7, 16, 10, 8};
...
// array of servos:
Servo servos[12] = {flhr, flhp, fls, frhr, frhp, frs, blhr, blhp, bls, brhr, brhp, brs};
...
int minPWMs[NUM_SERVOS] = {0, 0, 10, 0, 0, 0, 0, 0, 15, 0, 0, 15};
int maxPWMs[NUM_SERVOS] = {180, 180, 150, 180, 180, 135, 180, 180, 155, 180, 180, 145};
bool invertServo[NUM_SERVOS] = {false, false, true, true, true, false, true, false, true, false, true, false};

// Offsets to get theta = 0 for  each servo
int offsets[NUM_SERVOS] = {69, 38, 117, 69, 41, 92, 62, 37, 93, 69, 52, 96};
...
enum JScontrol_enum {H_V, FB_Y, P_R, TRANS_TROT_CONT, TRANS_TROT, ROTATE}; // determines which two axes the joystick controls. Cycled with the button
 ...
 * Locomotion
 */ 
enum gait_enum {trot_cont, trot, gallop, pronk};
gait_enum gait = trot;
...
{
  if (JScontrol != TRANS_TROT_CONT && JScontrol != TRANS_TROT && JScontrol != ROTATE) // Stationary rotations and translations
  {
...
void updateLegTrajectories()
{
  ...
  {
    /*
    * 4 steps, adjacent legs offset by two
    ...
    */
    ...
    int top = 125;
    int botFront = botFrontInitial;
    int botBack = 165;

    static float forward = 0;

    if (desiredGaitVelY > stopThresholdFB)
    { forward = 25+desiredGaitVelY; } //desiredGaitVelY;
    else if (desiredGaitVelY < -stopThresholdFB)
    { forward = -25+desiredGaitVelY; }
    else
    { forward = 0; }
    ...
    { sideways = 15+desiredGaitVelX; } //desiredGaitVelY;
    else if (desiredGaitVelX < -stopThresholdSideways)
    { sideways = -15+desiredGaitVelX; }
    else
    { sideways = 0; }
    ...
    if (flTraj.trajectoryComplete == true)
    {
      ...
      {
        case up:
          flTraj.interpolate(0,-sideways, 0,-forward, top,botFront, subPts);
          flTrot = front;
          break;
        case front:
          flTraj.interpolate(-sideways,0, -forward, 0, botFront,botFront, subPts);
          flTrot = middle;
          break;
        case middle:
          flTraj.interpolate(0,sideways, 0,forward, botFront,botFront, subPts);
          flTrot = back;
          break;
        case back:
          flTraj.interpolate(sideways,0, forward,0, botFront,top, subPts);
          flTrot = up;
          break;
        default:
          flTrot = up;
          Serial.println("state not used in trot_cont");
          break;
      }
    }
    ...
    if (frTraj.trajectoryComplete == true)
    {
      ...
      {
        case up:
          frTraj.interpolate(0,-sideways, 0,-forward, top,botFront, subPts);
          frTrot = front;
          break;
        case front:
          frTraj.interpolate(-sideways,0,-forward,0, botFront,botFront, subPts);
          frTrot = middle;
          break;
        case middle:
          frTraj.interpolate(0,sideways, 0,forward, botFront,botFront, subPts);
          frTrot = back;
          break;
        case back:
          frTraj.interpolate(sideways,0, forward,0, botFront,top, subPts);
          frTrot = up;
          break;
        default:
          frTrot = middle;
    ...
    if (blTraj.trajectoryComplete == true)
    {
      ...
      {
        case up:
          blTraj.interpolate(0,-sideways, 0,-forward, top,botBack, subPts);
          blTrot = front;
          break;
        case front:
          blTraj.interpolate(-sideways,0, -forward, 0, botBack,botBack, subPts);
          blTrot = middle;
          break;
        case middle:
          blTraj.interpolate(0,sideways, 0,forward, botBack,botBack, subPts);
          blTrot = back;
          break;
        case back:
          blTraj.interpolate(sideways,0, forward,0, botBack,top, subPts);
          blTrot = up;
          break;
        default:
          blTrot = middle;
    ...
    if (brTraj.trajectoryComplete == true)
    {
      ...
      {
        case up:
          brTraj.interpolate(0,-sideways, 0,-forward, top,botBack, subPts);
          brTrot = front;
          break;
        case front:
          brTraj.interpolate(-sideways,0,-forward,0, botBack,botBack, subPts);
          brTrot = middle;
          break;
        case middle:
          brTraj.interpolate(0,sideways, 0,forward, botBack,botBack, subPts);
          brTrot = back;
          break;
        case back:
          brTraj.interpolate(sideways,0, forward,0, botBack,top, subPts);
          brTrot = up;
          break;
        default:
          brTrot = up;
  ...
  else if (gait == trot)
  {
    ...
    */
    ...
    if (desiredGaitVelX > stopThresholdSideways)
    { sideways = initialSideThrow + desiredGaitVelX; } //desiredGaitVelY;
    else if (desiredGaitVelX < -stopThresholdSideways)
    ...
    if (buttonTapped)
    {
     switch(JScontrol)
     {
      ...
      case FB_Y:
        ...
        break;
      case P_R:
        JScontrol = TRANS_TROT;
        gait = trot;
        break;
//      case TRANS_TROT_CONT:
//        JScontrol = TRANS_TROT;
//        gait = trot;
//        break;
      case TRANS_TROT:
